...
int main() {
    ...
    for (int i = 0; i < 6; i++) {
        Vertex2D sv0 = screen[faces[i].v[0]]; Vertex2D sv1 = screen[faces[i].v[1]]; Vertex2D sv2 = screen[faces[i].v[2]]; Vertex2D sv3 = screen[faces[i].v[3]];
        int dx1 = sv1.x - sv0.x; int dy1 = sv1.y - sv0.y; int dx2 = sv2.x - sv0.x; int dy2 = sv2.y - sv0.y;
        if ((dx1 * dy2) - (dy1 * dx2) <= 0) continue;
        uint8_t col = (i + 1) * 40;
        draw_triangle(sv0, sv1, sv2, col); draw_triangle(sv0, sv2, sv3, col);
    }
    FILE *f = fopen("output2.ppm", "w");
    fprintf(f, "P3\n%d %d\n255\n", WIDTH, HEIGHT);
